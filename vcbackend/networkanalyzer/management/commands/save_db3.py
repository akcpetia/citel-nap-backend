...
def event_time_and_type(event):
    eventdatetime = datetime.datetime.fromisoformat(event['eventTime'])
...
class Command(S3Command):
    ...
    def handle(self, *args, **options):
        #s3://citel-nap/velocloud/db3/2023-2-6/1675714313.811725/device-260.json
        ...
        for network in credentials:
            ...
            for enterprise in enterprises.values():
                ...
                for edge in edges["result"]:
                    ...
                    #TODO handle when edge_events['result']['metaData']['more'] is True
                                    ...
                                    for matched_link in matched_links:
                                        ...
                                        for (ievent, event) in enumerate(all_events):
                                            if event['event'] == dead_name:
                                                assert ievent%2 == 0  # check that the event 0-based index is even
                                                death_s = [event]
                                                ...
                                                death_datetime = datetime.datetime.fromisoformat(death_s[0]['eventTime'])
                                                revival_datetime = datetime.datetime.fromisoformat(event['eventTime'])
